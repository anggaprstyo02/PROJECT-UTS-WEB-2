...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark sticky-top shadow">
  <div class="container-fluid">
    <span class="navbar-brand">Data Kampus</span>
  </div>
</nav>

<!-- Content -->
<div class="content">
  <h2 class="fw-bold mb-4">Kampus</h2>
  <a href="tambah.php" class="btn btn-primary mb-4"><i class="fas fa-plus me-2"></i>Tambah Kampus</a>
      ...
      <table class="table table-bordered table-hover table-striped align-middle text-center">
        ...
        <tbody>
          ...
          if (isset($koneksi) && $koneksi) {
            $result = $koneksi->query("SELECT * FROM kampus");
            while ($row = $result->fetch_assoc()) {
              echo "<tr>
                  <td class='text-center'>" . htmlspecialchars($row['id']) . "</td>
                  <td>" . htmlspecialchars($row['nama']) . "</td>
                  <td>" . htmlspecialchars($row['alamat']) . "</td>
                  <td class='text-center'>" . htmlspecialchars($row['latitude']) . "</td>
                  <td class='text-center'>" . htmlspecialchars($row['longitude']) . "</td>
            ...
            echo "<tr><td colspan='6' class='text-center text-muted'>Koneksi database gagal.</td></tr>";
          }
          ?>
        </tbody>
      </table>
