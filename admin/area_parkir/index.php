...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark sticky-top shadow">
  <div class="container-fluid">
    <span class="navbar-brand">Data Area Parkir</span>
  </div>
</nav>

<!-- Content -->
<div class="content">
  <h2 class="fw-bold mb-4">Area Parkir</h2>
  <a href="tambah.php" class="btn btn-primary mb-4"><i class="fas fa-plus me-2"></i>Tambah Area Parkir</a>
  ...
  <div class="card shadow-sm p-4">
    <div class="table-responsive">
      <table class="table table-bordered table-hover table-striped align-middle text-center">
        ...
        <tbody>
          <?php
          $query = "SELECT a.*, k.nama AS kampus_nama 
                    FROM area_parkir a 
          ...
          while ($row = $result->fetch_assoc()) {
              echo "<tr>
                  <td>{$row['id']}</td>
                  <td class='text-center fw-semibold'>{$row['nama']}</td>
                  <td><span class='badge bg-info text-dark'>{$row['kapasitas']} slot</span></td>
                  <td class='text-center text-muted'>{$row['keterangan']}</td>
                  <td><span class='badge bg-secondary badge-kampus'>{$row['kampus_nama']}</span></td>
                  <td>
                      <a href='edit.php?id={$row['id']}' class='btn btn-warning btn-sm me-1'><i class='fas fa-edit'></i></a>
                      <a href='hapus.php?id={$row['id']}' class='btn btn-danger btn-sm' onclick=\"return confirm('Yakin ingin menghapus data ini?');\"><i class='fas fa-trash-alt'></i></a>
                  </td>
              </tr>";
          }
          ?>
